...
- Floppy can hold several entries for one title, one per viewing, and shows the score on the most recently active entry that has one. Import reads that score. Export puts the rating on the newest entry, and also on a more recently active entry that shows a different score. A removal clears the score on every entry.
